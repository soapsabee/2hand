...
//    $strQuery = "select * from Factory_Master";	
 //   $result = mysqli_query($strQuery) or die(mysqli_error());
            $strQuery = "SELECT Product_Type,COUNT(*) AS NUM FROM product GROUP BY Product_Type";
            $result2 = mysqli_query($link,$strQuery) or die(mysqli_error()); 
    //Iterate through each factory
  if ($result2) {
        while($ors2 = mysqli_fetch_array($result2,MYSQLI_ASSOC)) {
            $strXML .= "<set label='" . $ors2['Product_Type'] . "' value='" . $ors2['NUM'] . "' />";
        }
    }
    mysqli_close($link);
    ...
    echo renderChart("../../FusionCharts/Column2D.swf", "", $strXML, "FactorySum", 600, 300, false, false);
?>
<BR><BR>
